...
This is the final handoff after a polish round on the first v2 draft. Compared to the earlier draft:

- **Masthead is compact, single-line.** Title dropped 32 → **26px**, cadence meta + Unclassified/OSINT stamp pinned to the right on the same baseline. No second-line cadence text.
- **Headline indicators are now a big horizontal bar**, not a small right-aligned vertical stack. The 4 cells (`Direction` / `7-day risk` / `Spillover` / `Ceasefire 30d`) sit BELOW the title as a `HeadlineBar` component with 28px display values and severity-tone coloring. This is the load-bearing at-a-glance read.
- **H1 is full-width** (max-width 24ch), dropped 52 → **48px**, no longer competes with a sidecar indicator column.
- **Section order reshuffled** so the scan path goes indicators → clocks → headlines → prose → events → analysis → reference:
  `§01 Multi-clock · §02 Key developments · §03 Executive summary · §04 Key events · §05 Strategic implications · §06 Evening flash · §07 Sources · §08 Casualties snapshot`
- **Casualties moved to §08 (the end).** Cumulative casualty numbers are reference, not lede — per editorial preference they now close the brief rather than sit mid-scroll.
- **Key developments now precedes Executive summary** (scan then read).
...
**Layout:** Top of page, full-width within the `1280px` max container. Compact single-row bar.

- **Top strip** (bottom: `3px double ink`, padding-bottom: `10px`, margin-bottom: `10px`, flex row, align-baseline, gap 20px):
  - Left (flex-shrink: 0, `whiteSpace: nowrap`):
    - `<span italic>ME</span> WAR` — Newsreader **26px** / weight 600 / letter-spacing -0.02em / color `ink`
    - ` · Intel Brief` — same size, weight 400, color `ink-mute`
  - Right (`margin-left: auto`, flex row, gap 12px, align-center):
    - Cadence meta: `Twice daily · 3D @8gara8` — mono 10px / 0.14em tracking / uppercase / `ink-mute` / `whiteSpace: nowrap`
    - UNCLASSIFIED · OSINT stamp (Stamp component, rotated -3°)
...
**Layout order** (key change from earlier drafts: the title is full-width, and the four top-level indicators are a **big horizontal bar BELOW the title** — not a small right-aligned stack. Scan order is: indicators → clocks → headlines → prose):

1. **Dateline** — mono 10px, 0.2em tracking, uppercase, color `accent`, margin-bottom 10px:
   `▮ Daily Brief · Day {NNN} · {weekday} {date}`
   (leading character is the red filled block U+25AE)

2. **H1 title** — full-width, NOT competing with indicators:
   - Newsreader, **48px**, line-height 1.05, letter-spacing -0.025em, weight 600, color `ink`
   - `text-wrap: balance`, `max-width: 24ch`, `margin: 0 0 28px`

3. **HeadlineBar** — prominent 4-up status bar, the first load-bearing visual after the title:
   - 4-column grid, equal columns, 0 gap, `2px solid ink` top+bottom borders, `1px solid rule-soft` on left/right, background `card-bg`
   - Each cell: `18px 20px` padding, `1px solid rule-soft` right border (except last)
   - Cell content:
     - Top row: `■ LABEL` — mono 10px, 0.16em tracking, uppercase, `accent` (the block is a `10×10` filled square in `accent`)
     - Value: Newsreader **28px**, weight 600, letter-spacing -0.01em, color keyed by severity tone — `accent` red for escalating/extreme/critical, `accent-amber` for mixed/conditional/degraded, `accent-green` for de-escalating/contained, `ink` otherwise. Value is uppercase.
   - Labels, in order: `DIRECTION` · `7-DAY RISK` · `SPILLOVER` · `CEASEFIRE · 30D`
   - Values come from `brief.direction`, `brief.risk7d`, `brief.spillover`, `${brief.ceasefire30d}%`

4. **Section rule**: `§01 · MULTI-CLOCK STATE · 6 indicators`

5. **6-clock gauge strip** (see Clocks section below) — 6-column grid, 10px gap

6. **Section rule**: `§02 · KEY DEVELOPMENTS · {N} items`
   (Key developments is now BEFORE exec summary — user scans bullets first, then reads prose.)
   - `<ol>` as a **2-column grid** (`1fr 1fr`, row-gap 0, column-gap 40px): each `<li>` is a 2-col grid `auto 1fr`, 14px gap, 12px 0 padding, `1px solid rule-soft` bottom.
     - Leading number: mono 11px, color `accent`, `01`, `02`... zero-padded, padding-top 2px
     - Text: sans **15px**, line-height 1.45, color `ink`, `text-wrap: pretty`

7. **Section rule**: `§03 · EXECUTIVE SUMMARY · {N} words`
   - Paragraph block (`max-width: 820px`): Newsreader **19px**, line-height 1.55, `text-wrap: pretty`, color `ink`
   - First letter is a **drop cap**: `float: left; font-size: 64px; line-height: 0.88; weight 600; margin-right: 10px; margin-top: 8px; color: accent; font-family: display`
   - End with `<div style="clear:both">` to contain the float.

8. **Section rule**: `§04 · KEY EVENTS (TIMELINE) · {N} entries`

9. **Events table** — full-width, inside a `1px solid rule-soft` card on `card-bg`:
...
10. **Section rule**: `§05 · STRATEGIC IMPLICATIONS · {N} threads`

11. **Implications** — 3-column grid, 28px gap. Each column:
...
12. **Flash note + Sources** (2-col grid `1fr 1fr`, 40px gap)
    - Left: Section rule `§06 · EVENING FLASH (18:00 TPE) · +9h delta`
      - Card: `1px solid rule-soft`, **left border `3px solid accent`**, `card-bg`, 14px padding
      - Text: Newsreader 15px line-height 1.55
    - Right: Section rule `§07 · SOURCES · {N} citations`
...
13. **Section rule**: `§08 · CASUALTIES SNAPSHOT · Cumulative · ±24–48h Δ`
    (Casualties is intentionally placed at the end of the brief — daily casualty deltas are reference material, not the lede.)

14. **Casualties block** — 4-column grid, 12px gap. One card per actor:
...
|-----|------|-------------|--------|----------|
| Masthead title | **26px** display | 1.0 | 600 | -0.02em |
| Brief H1 | **48px display** | 1.05 | 600 | -0.025em |
| HeadlineBar value | 28px display | 1.0  | 600 | -0.01em |
...
| `components/BriefHeader.tsx` | Replace with the Dateline + full-width H1 + **HeadlineBar** (4-up horizontal status bar). The old vertical right-aligned indicator stack was dropped — the bar reads faster at a glance. |
...
- `components/HeadlineBar.tsx` — NEW. The 4-cell status bar that sits between the H1 and the clocks on Today. Takes `{direction, risk7d, spillover, ceasefire30d}` and renders 4 equal cells with label/value and severity-tone color.
- `components/Pill.tsx` — normalized pill (still used inside the Events table `Dir.` column, Archive rows, and the legacy EscalationGauge MDX component)
...
└── reference/
    ├── ME War Intel Brief.html    ← **canonical source of truth**. Open in a browser; everything is inlined here.
    └── src/                       ← component sources extracted for easier reading; may drift slightly from the inlined HTML above. When in doubt, the HTML wins.
        ...
        ├── today.jsx              ← Masthead, TodayView, HeadlineBar, EventsTable, CasualtiesBlock, Sparkbar
...
- [ ] Homepage renders, in order: Masthead → Dateline+Title → **HeadlineBar (4-up horizontal status cells)** → 6-clock strip → Key Developments (2-col grid) → Executive Summary with drop-cap → Events table → Strategic Implications (3-up) → Flash + Sources (2-col) → Casualties 4-up.
- [ ] `/archive`, `/timeline`, `/casualties`, `/clocks`, `/about` match their specs.
- [ ] Brief H1 is 48px Newsreader weight 600, full-width, `text-wrap: balance`, `max-width: 24ch`.
- [ ] HeadlineBar shows 4 equal cells with 28px display values, severity-tone colored, between title and clocks strip.
- [ ] Casualties snapshot is positioned at the end (§08), after Flash + Sources.
